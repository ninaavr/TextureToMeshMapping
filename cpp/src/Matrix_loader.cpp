...
void Matrix_loader::load_seam(TexturedPolyhedron& tp, Eigen::SparseMatrix<double>& M) {
	...
	while (next != tp.seam->end()) {
		if (!are_sharing_vertex(*prev, *next)){//happens at end of a branch and beginning of another
				prev = next; ++next;
			}
		if (!seam_2branches((*prev)->vertex(), *tp.seam, true)) {
			//cout << "prev:     " << (*prev)->vertex()->id() << endl;
			//cout << "next:     " << (*next)->vertex()->id() << endl;
			...
			while (!are_sharing_vertex(*prev, *branch))
				++branch;
			//cout << "branch:     " << (*branch)->vertex()->id() << endl;
...
void Matrix_loader::load_seam_between(const Halfedge_handle& prev,
		const Halfedge_handle& next, const int side,
		Eigen::SparseMatrix<double>& M) {
	Halfedge_iterator hi = prev;
	Halfedge_iterator start = prev->prev_on_vertex();
	Vertex_handle v = prev->vertex();
	int sizeColumns = count_this_type();
	if (this_type[v]) {
		for (; hi != next->opposite() && (hi != start); hi =
				hi->next_on_vertex())
			load_seam_face(v, hi->face(), side, sizeColumns, M);
		//if (hi == start && hi != next->opposite()){
		//cout << hi->vertex()->id()<<endl;
		//cout << "Error:Matrix_Loader:seam is broken!" << endl;
		//exit(0);
		//}
	}
}

bool Matrix_loader::halfedge_is_between(const Halfedge_handle& this_h,
		const Halfedge_handle& prev, const Halfedge_handle& next) {
	bool is_between = false;
	Halfedge_iterator start = prev->prev_on_vertex();
	Halfedge_iterator hi;
	for (hi = prev; hi != next->opposite() && (hi != start);
...
void Matrix_loader::load_seam_face(Vertex_handle v, Facet_handle f,
		const int side, int sizeColumns, Eigen::SparseMatrix<double>& M) {
	int col;
	col = column[v][side];
...
bool Matrix_loader::are_sharing_vertex(Halfedge_handle& h1,
		Halfedge_handle& h2) {
...
int Matrix_loader::count_seam_neighbours(
		const TexturedPolyhedron::Vertex_handle& v) {
	int size = 0;
...
bool Matrix_loader::seam_2branches(const TexturedPolyhedron::Vertex_handle& vi, std::list<Halfedge_handle>& seam, const bool del) {
	int result = 0;
	typename std::list<Halfedge_handle>::iterator it;
	for (it = seam.begin(); it != seam.end() && (result < 2); ++it) {
		if ((*it)->vertex() == vi)
			++result;
		if (result == 2) {
			if(del)
