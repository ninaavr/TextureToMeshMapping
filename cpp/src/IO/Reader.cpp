	...
	while (fgets(line, 1024, fp)) {
		...
		if (line[0] == 'v'&& line[1] != 't'&& line[1] != 'n') {
			...
			if (!textures && !normals) {
                ...
                scanf(line, "%*s%d%d%d", &ids[0], &ids[1], &ids[2]);
                tris.push_back(ids[0]- 1);
                tris.push_back(ids[1] - 1);
                tris.push_back(ids[2] - 1);
...
void Reader::get_integers(const char *v, int* ival) {
	...
	sscanf(s.c_str(), "%*s %d %d %d %d %d %d %d %d %d", &ival[0], &ival[1], &ival[2], &ival[3], &ival[4], &ival[5], &ival[6], &ival[7], &ival[8]);
}