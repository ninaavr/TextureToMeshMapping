//============================================================================
// Name        : TexturedMeshProcessing.cpp
// Author      : ninaavr
// Version     :
...
/*template <class Refs, class T, class P, class Norm>
 void Textured_vertex<Refs, T, P, Norm>::setU(double input_v){
 m_v = input_v;
 }
 template <class Refs, class T, class P, class Norm>
 void Textured_vertex<Refs, T, P, Norm>::setU(double input_u){
...
public:
	...
	void compute_normals_per_vertex() {
		std::for_each(this->vertices_begin(), this->vertices_end(),
				Vertex_normal());
	}
	...
	void gl_draw_textured_triangles(bool smooth_shading, bool use_normals,
			const double scaling_tex_coordinates) {
		::glBegin(GL_TRIANGLES);
		Facet_iterator f = this->facets_begin();
		for (; f != this->facets_end(); f++)
			gl_draw_textured_facet(f, smooth_shading, use_normals,
					scaling_tex_coordinates);
		::glEnd();
	}

	void gl_draw_textured_facet(Facet_handle f, bool smooth_shading,
			bool use_normals, const double scaling_tex_coordinates) {
		// one normal per face
		...
		do {
			...
			::glTexCoord2d(u * scaling_tex_coordinates,
					v * scaling_tex_coordinates);
			::glVertex3d(p[0], p[1], p[2]);
		} while (++he != f->facet_begin());
	}

	//set only after vertices are added!
	void set_vertex_properties(std::vector<double> &normals, std::vector<double> &textures) {
		int i = 0;
		int index = 0;
		normalsExist = true;
		Vertex_iterator vi;
		for (vi = this->vertices_begin(); vi != this->vertices_end(); ++vi) {
			if (!normals.empty()) {
				vi->normal() = Vector(normals[i], normals[i + 1], normals[i + 2]);
			}
			vi->id() = index;
			++index;
			i += 3;
	...
	if (!textures.empty()) {
		texturesExist = true;
		for (vi = this->vertices_begin(); vi != this->vertices_end();
				++vi) {
			vi->u() = textures[i];
			vi->v() = textures[i + 1];
			i += 2;
